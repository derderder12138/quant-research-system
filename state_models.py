...
# --- 节点 1：真实 A 股数据获取 Agent ---
def data_fetcher_node(state: StockAgentState):
    print("-> 执行节点：[数据获取 Agent] 正在拉取 A 股真实数据...")
    ticker = state["ticker"] 
    try:
        ...
        if df.empty:
            raise ValueError(f"未找到代码 {ticker} 的数据，请检查代码是否正确。")
            
        df = df.tail(5)[['日期', '开盘', '最高', '最低', '收盘', '成交量']]
        df.set_index('日期', inplace=True)
        
        print(f"   状态：成功获取 {ticker} 的最新交易数据。")
        return {
            "data_fetch_success": True,
            "raw_history_data": df.to_dict(orient="index"),
            "error_message": ""
        }
    except Exception as e:
        print(f"   状态：数据获取异常！错误信息: {e}")
        return {
...
def check_data_status(state: StockAgentState):
    ...
    if state["data_fetch_success"]:
        print("   决策：数据获取成功，分流至技术面与基本面双 Agent 并行分析。")
        return "continue"
    else:
        print("   决策：数据获取失败，触发安全熔断，终止流程！")
        return "end"
...
def technical_analyst_node(state: StockAgentState):
    ...
    prompt = f"你是一位专业的技术分析师。请根据以下 A 股数据 ({ticker}) 给出简短技术分析，包含支撑位与趋势，控制在 100 字以内。\n数据: {raw_data}"
...
if __name__ == "__main__":
    ...
    if final_state["data_fetch_success"]:
        ...
        print(f"流程已熔断，原因：{final_state['error_message']}")